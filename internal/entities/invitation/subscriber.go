...
func (s *InvitationSubscriber) Start(ctx context.Context, stream nats.JetStreamContext, subject string) error {
	...
	_, err := stream.Subscribe(subjects.Invitations.Create, func(msg *nats.Msg) {
		...
		var evt events.InvitationCreateEvent
		...
		userEntity, err := userService.FindOrCreateInvitedUser(ctx, evt.Data.Email)
		if err != nil {
	...
	_, err2 := stream.Subscribe(subjects.Invitations.Accept, func(msg *nats.Msg) {
		var evt events.InvitationAcceptEvent
		if err := json.Unmarshal(msg.Data, &evt); err != nil {
			s.Logger.Errorw("Error unmarshaling invitation accept event", "error", err)
			return
		}
		invitationRepo := NewInvitationRepository(s.DB)
		inv, err := invitationRepo.FindByToken(ctx, evt.Data.Token)
		if err != nil {
			s.Logger.Errorw("Invitation not found for token", "token", evt.Data.Token, "error", err)
			return
		}
		...
		hash, err := utils.HashPassword(evt.Data.Password)
		if err != nil {
